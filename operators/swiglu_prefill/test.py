...
def main():
    ...
    golden_ref = generate_golden_reference(M=args.seq_len, K=args.embedding_dim, N=args.hidden_dim)
    
    operator = AIESwiGLUPrefill(seq_len=args.seq_len, embedding_dim=args.embedding_dim, hidden_dim=args.hidden_dim, prio_accuracy=bool(args.prio_accuracy))
    ...
    errors, latency_us, bandwidth_gbps = run_test(
        operator, input_buffers, output_buffers, intermediate_buffers, rel_tol=0.07, abs_tol=0.7
    )

    ref_2 = operator.read_buffer_as_torch('left_swished', (args.seq_len, args.hidden_dim)) * operator.read_buffer_as_torch('right', (args.seq_len, args.hidden_dim))
    ...
    ref_3 = operator.read_buffer_as_torch('intermediate', (args.seq_len, args.hidden_dim)) @ golden_ref['w_down'].T
