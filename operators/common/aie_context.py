...
class AIEContext:
    ...
    def prepare_runtime(self):
        ...
        for buffer_data in self.static_data_pool:
            logging.debug(f"Allocating static buffer with size {len(buffer_data)} bytes.")
            bo = pyxrt.bo(
        ...
        for op in self.operators:
            ...
            for kernel_name, (xclbin, xclbin_kernel_name, insts) in op.kernels.items():
                ...
                logging.debug(f"Allocating instruction buffer for {len(instructions)} instructions.")
                insts_bo = pyxrt.bo(
                    self.device_manager.device,
                    instructions.nbytes,
                    pyxrt.bo.cacheable,
                ...
                op.xrt_kernels[kernel_name] = (context, xrt_kernel, insts_bo, len(instructions))
            ...
            for kernel, *args in op.runlist:
                for arg in args:
                    ...
                    conflicting_args = {a for a in args if get_pool_sz(op.buffers[a]) == pool_sz} - {arg}
                    conflicting_buffers[arg] = conflicting_buffers.get(arg, set()).union(conflicting_args)
            ...
            # Setup runlist
            _, (first_xclbin, first_xclbin_kernel_name, _) = next(iter(op.kernels.items()))
            ...
            for i, (kernel_name, *buffer_args) in enumerate(op.runlist):
                this_context, xrt_kernel, insts_bo, insts_len = op.xrt_kernels[kernel_name]
