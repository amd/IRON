...
def generate_golden_reference(input_length, tile_size, group_size):
    ...
    per_tile_bytes = scale_size + per_tile_size  # Total bytes (uint8 elements) after processing each tile
    ...
    A = torch.rand(num_tiles * num_scale_factors, group_size, dtype=torch.bfloat16) * val_range
        ...
        for j in range(num_scale_factors):
            for k in range(group_size // 2):
                A_concat[i, j * (group_size // 2) + k] = torch.bitwise_or(
                    ...
                    torch.bitwise_and(A[i * num_scale_factors + j, 2 * k + 1], 0x0F) * 2**4,
                )
        for j in range(num_scale_factors):
            ...
            A_concat[i, per_tile_size + 2 * j + 1] = torch.bitwise_and(
                (scales[i * num_scale_factors + j].view(torch.uint16) / 2**8).to(torch.uint16),
                0xFF,
            )
