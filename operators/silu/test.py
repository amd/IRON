    ...
    for input_length in input_lengths:
        for num_aie_columns in range(1, max_aie_columns + 1):
            ...
            if check_length == input_length:
                names.append(f"silu_{num_aie_columns}_cols_{num_channels}_channels_{input_length}_tile_{tile_size}")
...
@pytest.mark.parametrize("input_length,num_aie_columns,num_channels,tile_size",
                         extensive_params,
                         ids=extensive_names)
def test_silu_extensive(input_length, num_aie_columns, num_channels, tile_size, aie_context):
