...
void softmax_reference(const std::bfloat16_t *input, std::bfloat16_t *output, int length)
{
    std::bfloat16_t max_val = input[0];
    float running = 0.0;
    for (int i = 1; i < length; i++) {
        std::bfloat16_t val = input[i];
        if (val > max_val) {
            max_val = val;
        }
    }
    for (int i = 0; i < length; i++) {
        float ez = (float)(exp(input[i] - max_val));
        running += ez;
        output[i] = static_cast<std::bfloat16_t>(exp(input[i] - max_val));
    }
    for (int i = 0; i < length; i++) {
        output[i] = static_cast<std::bfloat16_t>(output[i] / static_cast<std::bfloat16_t>(running));
    }
}
...
int main(int argc, const char *argv[])
{
    ...
    std::string dev = vm["dev"].as<std::string>();
    ...
    if (N != rows * cols) {
        std::cerr << "Length (" << N << ") must equal rows (" << rows << ") * cols (" << cols << ") = " << (rows * cols)
                  << std::endl;
        return 1;
    }
    ...
    double total_bytes = 3.0 * N * sizeof(std::bfloat16_t); // input and output
