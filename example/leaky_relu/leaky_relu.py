...
def my_leaky_relu(dev, size, num_columns, num_channels, tile_size, trace_size, alpha):
    xfr_dtype = bfloat16
...
if __name__ == "__main__":
    ...
    # Alpha parameter for Leaky RELU (default 0.01)
    p.add_argument(
        "-a",
        "--alpha",
        required=False,
        dest="alpha",
        default="0.01",
        help="Alpha parameter for Leaky RELU (default: 0.01)",
    )
    p.add_argument(
        "--output-file-path",
        "-o",
    ...
    alpha = bfloat16(float(opts.alpha))

    module = my_leaky_relu(dev, length, columns, channels, tile_size, trace_size, alpha)
