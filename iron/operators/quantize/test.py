    ...
    for input_length in input_lengths:
        for num_columns in range(1, max_aie_columns + 1):
            for num_channels in range(1, 3):  # 1 or 2 channels
                ...
                # Ensure tile_size is divisible by group_size
...
def test_quantize(
    ...
    # Allow ±1 tolerance for int8 rounding differences between AIE and x86
    # abs_tol=1.01 because verify_buffer uses >= (so 1.0 fails on exact ±1 diff)
    errors, latency_us, bandwidth_gbps = run_test(
        operator, input_buffers, output_buffers, rel_tol=0.01, abs_tol=1.01
    )
