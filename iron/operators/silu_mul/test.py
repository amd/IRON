...
from iron.operators.silu_mul.reference import generate_golden_reference
    ...
    for input_length in input_lengths:
        for num_aie_columns in range(1, max_aie_columns + 1):
            ...
            if tile_size * num_aie_columns != input_length:
                continue
            names.append(
                f"silu_mul_{num_aie_columns}_cols_{num_channels}_channels_{input_length}_tile_{tile_size}"
            )
...
def test_silu_mul(input_length, num_aie_columns, num_channels, tile_size, aie_context):
