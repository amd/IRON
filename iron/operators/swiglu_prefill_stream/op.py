...
class SwiGLUPrefillStream(OperatorSequence):
    ...
    Runtime buffers (``get_callable().get_buffer(name)``): ``input``, ``weights_1``
    (gate), ``weights_2`` (up), ``weights_3`` (down), ``output``. Building requires
    ...
    def __init__(
        ...
        super().__init__(
            ...
            runlist=[(block, "input", "weights_1", "weights_2", "weights_3", "output")],
            input_args=["input", "weights_1", "weights_2", "weights_3"],
            output_args=["output"],
...
class SwiGLUPrefillStreamK2(OperatorSequence):
    ...
    down-projection group fused into one full-ELF, with the hidden state ``h``
    kept on device between them. Same external buffers as
    :class:`SwiGLUPrefillStream`; the split is decided by the stream mapping
    (``make_swiglu_mapping(split_groups=True)``).
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            runlist=[
                (front, "input", "weights_1", "weights_2", "h"),
                (down, "h", "weights_3", "output"),
            ],
            input_args=["input", "weights_1", "weights_2", "weights_3"],
            output_args=["output"],
            extra_flags=["--dynamic-objFifos"],
