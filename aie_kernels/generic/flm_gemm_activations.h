...
// Branch-free activations over an aie::vector<bfloat16, N>, for flm_gemm's
// fused epilogue.
//
...
//
// Where that tanh comes from is the one part of flm_gemm that is genuinely
...
//
// This header was called nonlut_based_ops.h when flm_gemm was AIE2P-only, which
// stopped being an accurate name once AIE2 brought in the LUT; it is flm_gemm's
// only consumer, hence the rename rather than a second copy.
#ifndef __FLM_GEMM_ACTIVATIONS_H__
#define __FLM_GEMM_ACTIVATIONS_H__
#include <aie_api/aie.hpp>

#if __AIE_ARCH__ >= 21
#define FLM_GEMM_NATIVE_TANH 1
#else
#define FLM_GEMM_NATIVE_TANH 0
...
flm_gemm_tanh_bf16(aie::accum<accfloat, vec_size> x) {
#if FLM_GEMM_NATIVE_TANH
  return aie::tanh<bfloat16>(x.template to_vector<float>());
#else
  static_assert(vec_size == 16,
                "AIE2's LUT tanh is fixed at 16 lanes, which is flm_gemm's "
...
flm_gemm_sigmoid_bf16(aie::vector<bfloat16, vec_size> x) {
  ...
  aie::vector<bfloat16, vec_size> tanh_x_half =
      flm_gemm_tanh_bf16<vec_size>(x_mul_half);
...
template <int vec_size>
aie::vector<bfloat16, vec_size>
flm_gemm_silu_bf16(aie::vector<bfloat16, vec_size> x) {
  aie::vector<bfloat16, vec_size> sigmoid_x =
      flm_gemm_sigmoid_bf16<vec_size>(x);
...
template <int vec_size>
aie::vector<bfloat16, vec_size>
flm_gemm_gelu_bf16(aie::vector<bfloat16, vec_size> x) {
  ...
  aie::vector<bfloat16, vec_size> sigmoid_x_scaled =
      flm_gemm_sigmoid_bf16<vec_size>(x_scaled);
  return aie::mul(x, sigmoid_x_scaled);
}

#endif // __FLM_GEMM_ACTIVATIONS_H__
