...
// The 2x2 blocked mmul at the heart of mm_fused.cc.
//
// rowA/colA/colB count r x s (A), s x t (B) and r x t (C) sub-tiles, not
// elements. A is one contiguous buffer spanning every z slice, so it is indexed
// directly rather than re-based per slice, and the mmul takes no locks. B is
// laid out column-major over blocks -- block (i, j) at (j * colA + i) -- which
// is the order the operator's pack_B emits.
//
// Each iteration of the j loop keeps four accumulators live (C00/C01/C10/C11)
// over a 2x2 block of output tiles, so each pair of A loads and each pair of B
// loads feeds four macs. That 2x2 is load amortization, not loop unrolling:
// dropping to 1x1 would halve the arithmetic per load, and this kernel is
// load-port bound. It is also why the j and z loops step by two and cannot be
// rolled tighter without giving that up.
//
// There is one definition per B storage format, because the two are not both
// compilable on both architectures -- see the type selection below. They share
// mm_fused_store_2x2 and this documentation; the surrounding loop nest is
// written out in each rather than shared through a macro, which would hide the
// operand handling that is the only real difference between them.
...
// B's element type. bfp16ebs8 is usable only on AIE2P:
// __AIE_API_SCALAR_BFP_TYPES__ is defined in aie_api/detail/aie2p/config.hpp,
// mmul_bfp16_bfp16.hpp exists only under aie2p/, and on AIE2 aie_api/types.hpp
// declares `struct bfp16ebs8 {}` -- an empty placeholder that turns any use of
// it into a compile error rather than a slow fallback. So AIE2 keeps B in bf16,
// and both forms below are live: one per architecture, not one plus a spare.
...
__aie_inline void mm_fused_store_2x2(float *__restrict pC1,
                                     float *__restrict pC2,
                                     const Acc &C00,
                                     const Acc &C01,
                                     const Acc &C10,
                                     const Acc &C11)
...
// B arrives already quantized to bfp16ebs8.
//
// The hardware only multiplies bfp16, so the bf16 form below has to convert B
// inside every mac -- transpose, widen, to_v64bfp16ebs8. B is static weights,
// so pack_B does that conversion once on the host instead. The values are
// unchanged: this hoists a rounding that already happened rather than adding
// one. It also makes B 9 bytes per 8 elements instead of 16, which is the point
// -- the operator is data-movement bound.
//
// B is streamed rather than pointer-indexed because a block_vector cannot be
// aie::load_v'd, and because bfp16ebs8 pointer arithmetic counts bytes rather
// than blocks. TODO: index it directly once llvm-aie#1232 ("sizeof(bfp16ebs8)
// == 1, not 9") is fixed; open as of 2026-09-10.
template <unsigned rowA, unsigned colA, unsigned colB, unsigned r, unsigned s, unsigned t>
__aie_inline void mm_fused_mmul_2x2(const bfloat16 *__restrict pA, const bfp16ebs8 *__restrict pB, float *__restrict pC)
{
    ...
    for (unsigned z = 0; z < rowA; z += 2) {
        ...
        for (unsigned j = 0; j < colB; j += 2) {
            ...
            for (unsigned i = 0; i < colA; i++) {
                ...
                pA2 += sizeA;
                accA0 = A0;
                accA1 = mul_elem_64(A1, concat(broadcast_one_to_v32bfloat16(), broadcast_one_to_v32bfloat16()));
...
template <unsigned rowA, unsigned colA, unsigned colB, unsigned r, unsigned s, unsigned t>
__aie_inline void mm_fused_mmul_2x2(const bfloat16 *__restrict pA, const bfloat16 *__restrict pB, float *__restrict pC)
{
