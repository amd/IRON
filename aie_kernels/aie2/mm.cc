...
template <typename T_in,
          ...
          bool c_row_maj = true>
static inline void
matmul_vectorized_4x2_mmul(const T_in *__restrict pA, const T_in *__restrict pB, T_out *__restrict pC)
{
        ...
        for (unsigned j = 0; j < colB; j += 2)
...
#endif
            {
                ...
                if constexpr (c_row_maj) {
                    ...
                    aie::store_v(pC2, aie::transpose(C11.template to_vector<T_out>(), r, t));
                    pC2 += MMUL::size_C;
                    aie::store_v(pC1, aie::transpose(C20.template to_vector<T_out>(), r, t));
                    pC1 += MMUL::size_C;
                    aie::store_v(pC2, aie::transpose(C21.template to_vector<T_out>(), r, t));
                    pC2 += MMUL::size_C;
                    aie::store_v(pC1, aie::transpose(C30.template to_vector<T_out>(), r, t));
                    pC1 += MMUL::size_C;
                    aie::store_v(pC2, aie::transpose(C31.template to_vector<T_out>(), r, t));
                    pC2 += MMUL::size_C;
                }
            }
    };
...
template <typename T_in,
          ...
          bool b_row_maj = true,
          bool c_row_maj = true>
static inline void matmul_vectorized_4x4(const T_in *__restrict pA, const T_in *__restrict pB, T_out *__restrict pC)
{
        ...
        for (unsigned j = 0; j < colB; j += 4)
...
#endif
            {
                ...
                if constexpr (c_row_maj) {
                    ...
                    aie::store_v(pC2, aie::transpose(C11.template to_vector<T_out>(), r, t));
                    pC2 += MMUL::size_C;
                    aie::store_v(pC3, aie::transpose(C12.template to_vector<T_out>(), r, t));
                    pC3 += MMUL::size_C;
                    aie::store_v(pC4, aie::transpose(C13.template to_vector<T_out>(), r, t));
                    pC4 += MMUL::size_C;
                    ...
                    aie::store_v(pC2, aie::transpose(C21.template to_vector<T_out>(), r, t));
                    pC2 += MMUL::size_C;
                    aie::store_v(pC3, aie::transpose(C22.template to_vector<T_out>(), r, t));
                    pC3 += MMUL::size_C;
                    aie::store_v(pC4, aie::transpose(C23.template to_vector<T_out>(), r, t));
                    pC4 += MMUL::size_C;
                    ...
                    aie::store_v(pC2, aie::transpose(C31.template to_vector<T_out>(), r, t));
                    pC2 += MMUL::size_C;
                    aie::store_v(pC3, aie::transpose(C32.template to_vector<T_out>(), r, t));
                    pC3 += MMUL::size_C;
                    aie::store_v(pC4, aie::transpose(C33.template to_vector<T_out>(), r, t));
                    pC4 += MMUL::size_C;
                }
            }
    };
...
static inline void
matmul_vectorized_4x4x4_i16_i16(const int16 *__restrict pA, const int16 *__restrict pB, int16 *__restrict pC)
{
    constexpr int r = 4;
    constexpr int s = 4;
    constexpr int t = 4;
...
template <unsigned m, unsigned k, unsigned n>
static inline void
matmul_vectorized_4x4x4_i16_i32(const int16 *__restrict pA, const int16 *__restrict pB, int32 *__restrict pC)
{
    constexpr int r = 4;
    constexpr int s = 4;
    constexpr int t = 4;
...
template <unsigned m, unsigned k, unsigned n>
static inline void
matmul_vectorized_4x8x4_bf16_bf16(const bfloat16 *__restrict pA, const bfloat16 *__restrict pB, bfloat16 *__restrict pC)
{
    constexpr int r = 4;
    constexpr int s = 8;
    constexpr int t = 4;
...
template <unsigned m, unsigned k, unsigned n>
static inline void
matmul_vectorized_4x8x4_bf16_f32(const bfloat16 *__restrict pA, const bfloat16 *__restrict pB, float *__restrict pC)
{
    constexpr int r = 4;
    constexpr int s = 8;
    constexpr int t = 4;
...
template <unsigned m, unsigned k, unsigned n>
static inline void
matmul_vectorized_4x8x8_i8_i8(const int8 *__restrict pA, const int8 *__restrict pB, int8 *__restrict pC)
{
    constexpr int r = 4;
    constexpr int s = 8;
    constexpr int t = 8;
    ...
    return matmul_vectorized_4x2_mmul<int8, int8, (m / r), (k / s), (n / t), r, s, t, is_b_row_maj, is_c_row_maj>(
        pA, pB, pC);
}

template <unsigned m, unsigned k, unsigned n>
static inline void
matmul_vectorized_4x8x8_i8_i16(const int8 *__restrict pA, const int8 *__restrict pB, int16 *__restrict pC)
{
    constexpr int r = 4;
    constexpr int s = 8;
    constexpr int t = 8;
    ...
    return matmul_vectorized_4x2_mmul<int8, int16, (m / r), (k / s), (n / t), r, s, t, is_b_row_maj>(pA, pB, pC);
}

template <unsigned m, unsigned k, unsigned n>
static inline void
matmul_vectorized_4x8x8_i8_i32(const int8 *__restrict pA, const int8 *__restrict pB, int32 *__restrict pC)
{
    constexpr int r = 4;
    constexpr int s = 8;
    constexpr int t = 8;
    ...
    return matmul_vectorized_4x2_mmul<int8, int32, (m / r), (k / s), (n / t), r, s, t, is_b_row_maj>(pA, pB, pC);
}
